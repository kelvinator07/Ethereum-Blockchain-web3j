...
public class EthereumBlockchainApplication {

    private static final Logger LOGGER = LoggerFactory.getLogger(EthereumBlockchainApplication.class);

    private final Web3j web3j;

    public EthereumBlockchainApplication(Web3j web3j) {
        this.web3j = web3j;
    }

    public static void main(String[] args) {
        SpringApplication.run(EthereumBlockchainApplication.class, args);
    }
        ...
        web3j.transactionObservable().subscribe(tx -> {
            ...
            try {
                ...
                if (transactionCount.getTransactionCount().intValue() % 10 == 0) {
                    ...
                    EthGetTransactionCount tc = web3j.ethGetTransactionCount(
                            coinbase.getAddress(),
                            DefaultBlockParameterName.LATEST).send();

                    Transaction transaction = Transaction.createEtherTransaction(
                            coinbase.getAddress(),
                            tc.getTransactionCount(),
                            tx.getValue(),
